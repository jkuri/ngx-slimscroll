...
import {
  ...
  ISlimScrollOptions,
  SLIMSCROLL_DEFAULTS
} from "../classes/slimscroll-options.class";
import {
  ISlimScrollEvent,
  SlimScrollEvent
} from "../classes/slimscroll-event.class";
import {
  SlimScrollState,
  ISlimScrollState
} from "../classes/slimscroll-state.class";
...
export class SlimScrollDirective implements OnInit, OnChanges, OnDestroy {
  ...
  constructor(
    ...
    @Inject(SLIMSCROLL_DEFAULTS)
    @Optional()
    private optionsDefaults: ISlimScrollOptions
  ) {
  ...
  setup() {
    ...
    if (this.optionsDefaults) {
      this.options = new SlimScrollOptions(this.optionsDefaults).merge(
        this.options
      );
    } else {
    ...
    if (this.scrollEvents && this.scrollEvents instanceof EventEmitter) {
      const scrollSubscription = this.scrollEvents.subscribe(
        (event: SlimScrollEvent) => this.handleEvent(event)
      );
  ...
  handleEvent(e: SlimScrollEvent): void {
    ...
    if (e.type === "scrollToBottom") {
      ...
      e.type === "scrollToPercent" &&
      (e.percent >= 0 && e.percent <= 100)
    ) {
      const y = Math.round((bottom / 100) * e.percent);
      this.scrollTo(y, e.duration, e.easing);
    } else if (e.type === "scrollTo") {
      const y = e.y;
      if (y < 0) {
        this.scrollTo(0, e.duration, e.easing);
      } else if (y >= bottom) {
        this.scrollTo(bottom, e.duration, e.easing);
      } else {
        this.scrollTo(y, e.duration, e.easing);
      }
  ...
  initGrid(): void {
    ...
    this.renderer.setStyle(
      grid,
      "border-radius",
      `${this.options.gridBorderRadius}px`
    );
  ...
  initBar(): void {
    ...
    this.renderer.setStyle(
      bar,
      "border-radius",
      `${this.options.barBorderRadius}px`
    );
  ...
  getBarHeight(): void {
    ...
    const barHeight =
      Math.max((elHeight / this.el.scrollHeight) * elHeight, 30) + "px";
  ...
  scrollTo(y: number, duration: number, easingFunc: string): void {
    ...
    const barHeight = Math.max(
      (this.el.offsetHeight / this.el.scrollHeight) * this.el.offsetHeight,
      30
    );
    ...
    const scroll = (timestamp: number) => {
      ...
      if (paddingBottom === 0) {
        const delta = Math.round(
          Math.round(this.el.clientHeight * percentScroll) - barHeight
        );
        if (delta > 0) {
  ...
  scrollContent(y: number, isWheel: boolean, isJump: boolean): null | number {
    ...
    if (isWheel) {
      delta =
        parseInt(getComputedStyle(this.bar).top, 10) +
        ((y * 20) / 100) * this.bar.offsetHeight;
    ...
    percentScroll =
      parseInt(getComputedStyle(this.bar).top, 10) /
      (this.el.offsetHeight - this.bar.offsetHeight);
